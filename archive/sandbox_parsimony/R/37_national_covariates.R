# =============================================================================
# sandbox_parsimony/R/37_national_covariates.R
#
# Covariate sources for the national (country-survey) models in 31 and 33.
#
# WHY THIS EXISTS
# ---------------
# 31_national_model.R was written as a self-contained viability check and pulled
# its own 17 WDI indicators from the World Bank API, chosen a priori on the same
# causal story as the curated Admin-2 set. That was never a considered parsimony
# decision -- the project already holds a national country x year panel
# (`mn-proxies/data/national/combined_dataset.dta`, 5,433 columns, 61 countries,
# 1950-2025) and nothing was reading it. This module makes the covariate source
# a switch so the two can be compared on the same folds.
#
# WHAT THE PANEL ACTUALLY CONTAINS
# --------------------------------
# 5,433 columns is a misleading headline. The panel is a full country x year
# grid, but most columns are DHS STATcompiler indicators that exist only in DHS
# survey years, so the median column is 5.5% populated over 1990-2022. Carrying
# the nearest value within +/- 5 years -- DHS rounds run about that far apart --
# lifts that: 104 columns reach 90% coverage, 280 reach 70%, 1,375 reach 50%.
# The usable pool is therefore a few hundred to ~1,400 depending on where the
# threshold is set, not 5,433. `min_coverage` is the knob and it is reported.
#
# ANAEMIA COLUMNS ARE EXCLUDED BY DEFAULT
# ---------------------------------------
# The panel carries DHS anaemia and haemoglobin indicators. Predicting a VMNIS
# micronutrient-deficiency prevalence from a measured anaemia prevalence is a
# different and much weaker claim than predicting it from development proxies --
# it is close to predicting a deficiency from a deficiency. They are dropped
# unless `drop_anaemia = FALSE`, and the count dropped is always reported.
# =============================================================================
...
PANEL_CACHE <- "sandbox_parsimony/out/panel_national.rds"
WDI_CACHE   <- "sandbox_parsimony/out/wdi_national.rds"
