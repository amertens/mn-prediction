...
full_vars=c(gee_vars, ihme_vars, map_vars)

gee_vars_gambia <- colnames(df_gambia)[grepl("gee_", colnames(df_gambia))|grepl("ihme_", colnames(df_gambia))|grepl("MAP_", colnames(df_gambia))]
gee_vars_ghana <- colnames(df_ghana)[grepl("gee_", colnames(df_ghana))|grepl("ihme_", colnames(df_ghana))|grepl("MAP_", colnames(df_ghana))]
gee_vars_malawi <- colnames(df_malawi)[grepl("gee_", colnames(df_malawi))|grepl("ihme_", colnames(df_malawi))|grepl("MAP_", colnames(df_malawi))]
gee_vars_sierraleone <- colnames(df_sierraleone)[grepl("gee_", colnames(df_sierraleone))|grepl("ihme_", colnames(df_sierraleone))|grepl("MAP_", colnames(df_sierraleone))]

gee_vars=full_vars[full_vars %in% gee_vars_gambia &
                     full_vars %in% gee_vars_ghana &
                     full_vars %in% gee_vars_malawi &
                     full_vars %in% gee_vars_sierraleone]
length(gee_vars)
...
saveRDS(resfull, file = here("results/models/res_GW_transportability_SL_women_vitA_geospatial_only.rds"))


res_ghana=try(DHS_SL(d=df_ghana, outcome="rbp", population="women", Xvars=Xvars, id=id, folds=Vfolds, CV=CVsetting, sl=slmodel))
saveRDS(res_ghana, file = here("results/models/res_GW_Ghana_SL_women_vitA_geospatial_only.rds"))

res_gambia=try(DHS_SL(d=df_gambia, outcome="rbp", population="women", Xvars=Xvars, id=id, folds=Vfolds, CV=CVsetting, sl=slmodel))
saveRDS(res_ghana, file = here("results/models/res_GW_Gambia_SL_women_vitA_geospatial_only.rds"))

res_malawi=try(DHS_SL(d=df_malawi, outcome="rbp", population="women", Xvars=Xvars, id=id, folds=Vfolds, CV=CVsetting, sl=slmodel))
saveRDS(res_malawi, file = here("results/models/res_GW_Malawi_SL_women_vitA_geospatial_only.rds"))
...
saveRDS(res_sierraleone, file = here("results/models/res_GW_SierraLeone_SL_women_vitA_geospatial_only.rds"))
