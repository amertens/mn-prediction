...
# ---- Data ----
res = readRDS(here("results/models/res_bin_GW_Ghana_SL_mom_iron_V2.rds"))

d <- readRDS(here("data/IPD/Ghana/Ghana_merged_dataset.rds"))

d <- d %>% select(dataid, lat, lon, gw_wIDAdjBrinda, gw_cn, gw_Strata, gw_sWeight) %>% rename(gw_EACode=gw_cn)
d<-left_join(d, res$res, by = "dataid")

summary(res$res$yhat_full)
prop.table(table(d$gw_wIDAdjBrinda))
d$pred_Y <- ifelse(d$yhat_full >= 0.24, 1, 0)
prop.table(table(d$pred_Y))
...
outcomes <- c("gw_wIDAdjBrinda", "pred_Y")

adm1_prev <- compute_admin1_prev(d_nog, outcomes) %>%
  mutate(outcome = recode(outcome,
                          gw_wIDAdjBrinda = "Survey-based iron deficiency\n(BRINDA adj.)",
                          pred_Y         = "Predicted iron deficiency"
  ))
...
poly_adm1_joined$outcome <- factor(poly_adm1_joined$outcome, levels=c("Predicted iron deficiency","Survey-based iron deficiency\n(BRINDA adj.)"))
...
p_prev <- ggplot(poly_adm1_joined) +
  ...
  labs(
    title    = "Ghana 2017 Admin-1 region women's iron deficiency prevalences",
