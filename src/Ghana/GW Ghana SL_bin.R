...
Xvars_full = c("Admin1","Admin2","gw_month",gw_vars,dhs_vars, mics_vars, ihme_vars, lsms_vars, map_vars, wfp_vars, flunet_vars, gee_vars)
Xvars_gw = c("Admin1","Admin2","gw_month",gw_vars)
Xvars = c("Admin1","Admin2","gw_month",dhs_vars, mics_vars, ihme_vars, lsms_vars, map_vars, wfp_vars, flunet_vars, gee_vars)
...
df_child_VitA <- df %>% select(gw_cVAD, gw_cnum, !!(Xvars_full)) %>% as.data.frame() %>% filter(!is.na(df$gw_childid), !is.na(gw_cVAD))
df_mom_VitA <- df %>% select(gw_wVAD, gw_cnum, !!(Xvars_full)) %>% as.data.frame() %>% filter(is.na(df$gw_childid), !is.na(gw_wVAD))
...
save(df_child_VitA, df_mom_VitA, df_child_iron, df_mom_iron, df_mom_b12, df_mom_folate,
     file=here("data/IPD/Ghana/clean_mn_data_Ghana.rdata"))
