...
d$gw_EACode <- as.numeric(d$gw_EACode)
...
d_clust <- d %>% group_by(gw_cn) %>% mutate(y=mean(gw_cVAD, na.rm=T)) %>% dist()
...
gee_a2_path <- here("data/GEE/Ghana_2017_admin2_gee.csv")
if (file.exists(gee_a2_path)) {
  ...
  warning("Admin2 GEE CSV not found — run scripts/build_gee_admin2.R Ghana")
}
...
#-------------------------------------------------------------------------------
espen_path <- here("data/ESPEN/Ghana_espen_admin2.csv")
if (file.exists(espen_path)) {
  ...
  message("ESPEN CSV not found — run scripts/build_espen_admin2.R Ghana (optional domain)")
}
...
#-------------------------------------------------------------------------------
spam_path <- here("data/MapSPAM/Ghana_spam_admin2.csv")
if (file.exists(spam_path)) {
  ...
  message("MapSPAM CSV not found — run scripts/build_mapspam_admin2.R Ghana (optional domain)")
}
...
#-------------------------------------------------------------------------------
for (.nd in list(list(v="vas", path=here("data/VAS/Ghana_vas_admin2.csv")),
                 list(v="fao", path=here("data/FAOSTAT/Ghana_fao_admin2.csv")),
                 list(v="fpn", path=here("data/FPN/Ghana_fpn_admin2.csv")))) {
...
# Admin1-level IHME predictors (built by IHME_clean.R via build_ihme_admin1).
source(here("src/IHME/build_ihme_admin1.R"))
df <- merge_ihme_admin1(
  df,
  admin1_csv  = here("data/IHME/ghana_2017_merged_IHME_admin1_data.csv"),
  admin1_col  = "Admin1"
)
...
saveRDS(metadata, here("metadata/ghana_variable_categories.rds"))
