...
# create log1p versions for skewed outcomes
skewed <- c("ferritin", "stfr", "rbp", "crp", "agp")
df_women    <- logify(df_women,    intersect(skewed, names(outcomes_women)))
df_children <- logify(df_children, intersect(skewed, names(outcomes_child)))

# convenience: Hb in g/L as well
if (has_col(df_women, "wm_whbc"))    df_women  <- df_women  %>% mutate(hb_gl = wm_whbc * 10)
if (has_col(df_children, "gchb"))    df_children<- df_children%>% mutate(hb_gl = gchb * 10)
...
print(plot_outcomes(df_women, outcomes_women, "Women outcomes"))

cat("\n==== CHILDREN: outcome summary ====\n")
print(summarize_outcomes(df_children, outcomes_child))
print(plot_outcomes(df_children, outcomes_child, "Children outcomes"))
...
bench_women %>% select(outcome, model, mse) %>% arrange(outcome, mse) %>% print(n = Inf)

cat("\n==== CHILDREN: cross-validated MSE ====\n")
bench_children %>% select(outcome, model, mse) %>% arrange(outcome, mse) %>% print(n = Inf)
