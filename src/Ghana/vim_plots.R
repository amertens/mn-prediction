...
#test
  ...
  varimp <- importance(
    ...
    covariate_groups = list("MN survey" = metadata$variable_sources$gw_vars[metadata$variable_sources$gw_vars %in% res$Xvars]),
  )


res_vim_diff_data_sources=try(calc_importance(res$sl_fit,  eval.fun = loss_squared_error, importance.metric="difference",
                                              covariate.groups=list("MN survey" = metadata$variable_sources$gw_vars[metadata$variable_sources$gw_vars %in% res$Xvars],
                                                                    "DHS" = metadata$variable_sources$dhs_vars[metadata$variable_sources$dhs_vars %in% res$Xvars],
                                                                    "MICS" = metadata$variable_sources$mics_vars[metadata$variable_sources$mics_vars %in% res$Xvars],
                                                                    "IHME" = metadata$variable_sources$ihme_vars[metadata$variable_sources$ihme_vars %in% res$Xvars],
                                                                    "LSMS" = metadata$variable_sources$lsms_vars[metadata$variable_sources$lsms_vars %in% res$Xvars],
                                                                    "WFP" = metadata$variable_sources$wfp_vars[metadata$variable_sources$wfp_vars %in% res$Xvars],
                                                                    "FluNet" = metadata$variable_sources$flunet_vars[metadata$variable_sources$flunet_vars %in% res$Xvars],
                                                                    "GEE" = metadata$variable_sources$gee_vars[metadata$variable_sources$gee_vars %in% res$Xvars])))
