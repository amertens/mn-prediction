...
adm2_prev = readRDS(here("data", "IPD", "Ghana", "Ghana_GMS_admin1_prevalence.rds"))

true_admin1_prev <- adm2_prev %>%
  filter(outcome == "Child VAD") %>%
  select(Admin1, prev, prev_upp ) %>%
...
table(true_admin1_prev$true_cat)
table(true_admin1_prev$true_cat_ub)
...
compute_adm1_prev <- function(data, outcomes) {
...
N_per_cluster =d_nog%>% group_by(Admin1) %>% summarise(N=n())

pred_adm1_prev <- compute_adm1_prev(d_nog, "pred_Y")
colnames(pred_adm1_prev) <- c("Admin1","pred_prev",  "pred_prev_se", "pred_prev_low", "pred_prev_upp", "outcome")
pred_adm1_prev <- pred_adm1_prev %>%
  select(Admin1, pred_prev, pred_prev_upp ) %>%
  #mild  ≥2%–<10% moderate  ≥10%–<20%b severe  ≥20%
...
table(true_admin1_prev$true_cat )
table(pred_adm1_prev$pred_cat )

res <- left_join(true_admin1_prev, pred_adm1_prev, by="Admin1")
res <- left_join(res, N_per_cluster, by="Admin1")
...
res %>% filter(true_cat=="None" & pred_cat=="Severe")
