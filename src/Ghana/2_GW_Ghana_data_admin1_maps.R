...
poly.adm <- geodata::gadm(country="GH", level=1, path=tempdir())
poly.adm <- sf::st_as_sf(poly.adm) %>% select(NAME_1) %>% rename(Admin1 = NAME_1)
...
# --- Helper: compute admin-1 prevalence for outcomes ---
...
adm1_prev <- compute_adm1_prev(d_nog, outcomes)

# --- Relabel outcomes for clarity ---
adm1_prev <- adm1_prev %>%
  dplyr::mutate(outcome = dplyr::recode(outcome,
                                        gw_cIDAdjBrinda = "Child iron deficiency",
                                        gw_wIDAdjBrinda = "Women iron deficiency",
                                        gw_cVAD         = "Child VAD",
                                        gw_wFolateDef   = "Women folate deficiency",
                                        gw_wB12Def      = "Women B12 deficiency",
                                        gw_wVADAdjThurn = "Women VAD"
  ))


#save outcomes for merge with predictors
saveRDS(adm1_prev, here("data", "IPD", "Ghana", "Ghana_GMS_admin1_prevalence.rds"))
...
p_prev_iron <- ggplot(poly_adm_joined %>% filter(outcome=="Child iron deficiency")) +
  ...
  labs(
    title    = "Child Iron Deficiency:\nGhana Micronutrient Survey 2017"  ) +
