...
ihme <- read_dta(here("data/IHME/IHME_Sierra_Leone_data.dta")) %>% filter(adm0_name=="Sierra Leone" & year==2012) %>%
...
# Save result
write_csv(df_wide_tidyr, here("data/IHME/SierraLeone_2012_merged_IHME_data.csv"))
...
build_ihme_admin1(
  ...
  year_filter  = 2012L,
  out_path     = here("data/IHME/SierraLeone_2012_merged_IHME_admin1_data.csv")
)
