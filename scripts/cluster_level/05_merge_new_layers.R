...
CDIR <- "data/covariates/cluster"
P  <- read.csv(file.path(CDIR, "predictors_cluster.csv"), check.names = FALSE)
MD <- read.csv(file.path(CDIR, "predictors_cluster_metadata.csv"), check.names = FALSE)
for (f in c("predictors_cluster.csv", "predictors_cluster_metadata.csv"))
...
blocks <- list(
  ...
  list(file = "predictors_cluster_livestock.csv", domain_fn = function(v) "Livestock density", role = "static", source = "GLW4 2015"),
...
for (b in blocks) {
  f <- file.path(CDIR, b$file); if (!file.exists(f)) { cat("  missing", b$file, "\n"); next }
...
write.csv(P, file.path(CDIR, "predictors_cluster.csv"), row.names = FALSE); write.csv(MD, file.path(CDIR, "predictors_cluster_metadata.csv"), row.names = FALSE)
