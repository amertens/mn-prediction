# =============================================================================
# scripts/accuracy_impact/wsa_predictor_consistency.R
#
# WS-A. Does ANY predictor carry a consistent association across the four
# surveys and the outcomes, with proper power?
#
# WHY THIS AND NOT THE FDR SCREEN ALREADY IN THE PROJECT
# ------------------------------------------------------
# Section 3 reports that no predictor survives FDR control in any of the 24
# cells (results/tables/bivariate_fdr.csv). That screen tests each predictor
# SEPARATELY IN EACH CELL, so it asks whether one predictor beats multiplicity
# inside a single country-outcome with 14 to 87 districts. A predictor with a
# real but modest association everywhere fails that test in every cell while
# being exactly what the project is looking for.
#
# This asks the question the other way round: pool the association ACROSS cells
# and countries, and test the pooled quantity. That is the analysis with power.
#
# WHAT IS DONE TO EACH ASSOCIATION BEFORE POOLING
#   scale          district-level design-weighted prevalence on the LOGIT scale,
#                  because a correlation with a proportion near 0 or 1 is
#                  compressed and four of these cells sit near 2 percent
#   two families   marginal, and region-partialed (predictor and outcome each
#                  residualised on Admin-1 within country). The second is the
#                  one that matters: WS4a showed skill is highest at Admin-1 and
#                  WS-E2 tests whether anything survives within region.
#   disattenuation divide the observed correlation by the cell's empirical
#                  r_max from reliability_empirical.csv before the Fisher z,
#                  capping at 0.99. An undisattenuated column is carried too,
#                  because disattenuation inflates variance and a reader should
#                  see both.
#
# HOW POOLING RESPECTS THE CLUSTERING
# -----------------------------------
# Outcomes within a survey share respondents, fieldwork and assay batch, so the
# 16 cells are NOT 16 independent tests. Country is the clustering unit: average
# the z values within a country first, then combine the four country values with
# a random-effects meta-analysis. Four clusters is few, which is why the
# analytic q-values are reported beside a PERMUTATION calibration.
#
# THE PERMUTATION NULL
# --------------------
# District outcomes are permuted WITHIN REGION WITHIN COUNTRY. That preserves
# the between-region structure, which WS4a showed carries most of what signal
# exists, and destroys only the within-region predictor-outcome link. A null
# that permuted across regions would be too easy to beat and would manufacture
# significance from the between-region socioeconomic axis.
#
#   PROFILE=smoke   Ghana child_iron only, 50 permutations
#   WSA_PERM=200    override the permutation count
#
#   Rscript scripts/accuracy_impact/wsa_predictor_consistency.R
# -> results/tables/predictor_consistency_meta.csv
# -> results/tables/predictor_consistency_top30.csv
# -> results/figures/predictor_consistency.png
# =============================================================================
...
for (cn in names(cfgs)) {
  ...
  for (ocn in ocs) {
    ...
    keep <- apply(X, 2, function(z) sum(is.finite(z)) == length(z) && stats::sd(z) > 0)
    ...
    cat(sprintf("  cell %-13s %-13s districts=%3d usable predictors=%3d r_max=%s\n",
...
readr::write_csv(out, file.path(TDIR, sprintf("predictor_consistency_meta%s.csv", SUF)))
...
top <- out |> filter(cellset == "shared") |> group_by(family) |>
  arrange(desc(abs(z))) |> slice_head(n = 30) |> ungroup()
readr::write_csv(top, file.path(TDIR, sprintf("predictor_consistency_top30%s.csv", SUF)))
...
png(file.path(FDIR, sprintf("predictor_consistency%s.png", SUF)),
...
cat(sprintf("\n-> %s\n", file.path("results", "figures",
                                   sprintf("predictor_consistency%s.png", SUF))))
