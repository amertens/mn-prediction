...
about_content <- div(
  ...
  p("Predictions come from SuperLearner ensemble machine learning models ",
    ...
    "surveys). Uncertainty is quantified using split-conformal prediction ",
    "intervals targeting 95% marginal coverage."),
  h6("Citation"),
...
glossary_content <- div(
  ...
  tags$dl(
    ...
    tags$dt("Conformal prediction interval"),
    tags$dd("A distribution-free 95% prediction interval. Unlike bootstrap ",
            "or parametric intervals, it provides coverage guarantees ",
            "without assuming a specific noise distribution."),
    ...
    tags$dd("Population living in districts whose local prevalence is ",
            "materially different from the country average. Quantifies ",
            "the cost of relying on national-level estimates for ",
            "sub-national targeting.")
  )
)
