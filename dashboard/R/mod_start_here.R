    ...
    output$example <- renderUI({
      ...
      tagList(
        ...
        if (is.finite(cap)) p(sprintf(paste("Sent to the fifth of districts the model ranks worst, a programme would reach about %s of Ghana's vitamin A deficient",
                                             "children; the survey's own regional averages reach %s. For this cell the two are a tie; across all 24 cells the model's margin is %s against %s."),
                                       fmt_pct(cap, 1), fmt_pct(cap_jk, 1), fmt_pct(Q$cap_index, 0), fmt_pct(Q$cap_jk, 0))),
