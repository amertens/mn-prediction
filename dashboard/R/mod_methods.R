  ...
  layout_columns(
    ...
    card(card_header("The surveys"),
         card_body(reactableOutput(ns("outcomes")),
                   p(style = "font-size:0.9em; color:#555;", "Vitamin A status is retinol-binding protein below 0.70 micromol per litre after",
                     " one BRINDA inflammation adjustment applied to all four surveys (low RBP, not a deficiency count); iron",
                     " deficiency is each survey's own inflammation-adjusted ferritin indicator below 12 (children) or 15 (women)",
                     " microgram per litre, never iron-deficiency anaemia. Children are 6 to 59 months and women are non-pregnant,",
                     " 15 to 49, as the survey reports tabulate them. Sierra Leone's child data as supplied cover the survey's",
                     " anaemic children only. No variable from the biomarker survey itself is used as a predictor, and any",
