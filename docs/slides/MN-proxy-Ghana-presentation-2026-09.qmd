...
::: {.notes}
Leave-one-country-out ablation, biomarker level, 22 cells. Only soil and climate are load-bearing; ten survey-derived domains cost nothing to remove, or help. A fixed climate + soil index transports better than the full vocabulary at both tiers (0.37 vs 0.26 at district level; 0.45 vs 0.31 regional). It was found on the same four countries it is scored on, so it enters the next countries as a pre-registered prediction, not a result.
:::
...
::: {.notes}
The January deck's "which data sources contribute" question, redone under the protocol: drop every column from a source, rebuild the domain PCs, score transport to a held-out country. Soil and Earth Engine layers carry it; dropping the 138 DHS/MICS columns IMPROVES transport (0.26 to 0.31 on the level, 0.16 to 0.23 on prevalence). The January version scored one country's in-sample change in MSE; this one scores transport.
:::
...
- Across borders, rankings transport and levels do not: about 0.3 at both tiers, 0.37 and 0.45 from climate and soil alone.
